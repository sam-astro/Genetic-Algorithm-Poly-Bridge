...
public class DebugGUI : MonoBehaviour
{
    ...
    [SerializeField] Color backgroundColor = new Color(0f, 0f, 0f, 0.9f);
    ...
    public static void SetGraphProperties(object key, string label, float min, float max, int group, Color color, bool autoScale, int xPos, int yPos)
    {
        if (GraphsEnabled)
            Instance.InstanceSetGraphProperties(key, label, min, max, group, color, autoScale, xPos, yPos);
    }
    ...
    private void InstanceSetGraphProperties(object key, string label, float min, float max, int group, Color color, bool autoScale, int xPos, int yPos)
    {
        if (!graphDictionary.ContainsKey(key))
            InstanceCreateGraph(key);
    ...
    void DrawGraphs()
    {
        ...
        {
            GUI.Box(new Rect(
                graphContainers[i].xPos,
                group * graphBlockHeight + graphContainers[i].yPos,
                graphLabelWidth + 5,
                graphHeight
            ), "", boxStyle);
            i++;
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            ...
            //float minMaxXOffset = width + 15;

            if (graphGroupBoxesDrawn.Add(graph.group))
            {
                GUI.Box(new Rect(graph.xPos + graphLabelWidth + 5, 0 + graphBlockHeight * graph.group + graph.yPos, graphWidth, graphHeight), "", boxStyle);
            }

            graph.Draw(new Rect(graph.xPos + graphLabelWidth + 5, 0 + graphBlockHeight * graph.group + graph.yPos, graphWidth, graphHeight));
        }
        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    string minText = graph.editMin.ToString("F2");
                    string maxText = graph.editMax.ToString("F2");
                    Vector2 textSize = minMaxTextStyle.CalcSize(labelGuiContent);
                    labelGuiContent.text = minText;
                    float width = textSize.x;
                    ...
                    // Max
                    labelGuiContent.text = graph.editMax.ToString("F2");
                    minMaxXOffset += width + 5;
                    GUI.Label(new Rect(graph.xPos, groupOrigin + graph.yPos, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.editMin.ToString("F2");
                    GUI.Label(new Rect(graph.xPos, groupOrigin + graphHeight - textSize.y + graph.yPos, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    ...
                    GUI.Label(new Rect(graph.xPos, yOffset + graph.yPos, xOffset, graphHeight), labelGuiContent);
        ...
        foreach (var group in graphGroupBoxesDrawn)
        {
            ...
            if (
                mousePos.x < graphContainers[group].xPos + graphWidth + graphLabelWidth && mousePos.x > graphContainers[group].xPos + graphLabelWidth &&
                mousePos.y > group * graphBlockHeight + graphContainers[group].yPos && mousePos.y < group * graphBlockHeight + graphHeight + graphContainers[group].yPos
            )
            {
                ...
                GUI.Box(new Rect(mousePos.x, group * graphBlockHeight + graphContainers[group].yPos, 1, graphHeight), "", boxStyle);
                ...
                float yOffset = graphContainers[group].yPos;
                foreach (GraphContainer graph in graphDictionary.Values)
                {
                    if (graph.group == group)
                    {
    ...
    class GraphContainer
    {
        ...
        public float min = 0;
        // Value at the top of the graph
        public float editMax = 1;
        // Value at the bottom of the graph
        public float editMin = 0;
        ...
        {
            if (this.editMin == min && this.editMax == max) return;
        ...
        public void Push(float val)
        {
            ...
            {
                target.SetPixel(target.width-1, i, Color.clear);
            }
            ...
            int y0 = (int)(Mathf.InverseLerp(editMin, editMax, value) * graphHeight);
            int y1 = (int)(Mathf.InverseLerp(editMin, editMax, nextVal) * graphHeight);

            // Prevent wraparound to zero
            y0 = y0 >= graphHeight ? graphHeight - 1 : y0;
            y1 = y1 >= graphHeight ? graphHeight - 1 : y1;

            DrawLine(target, target.width-2, y0, target.width-1, y1, color);
        }
        ...
        private void RegenerateGraph()
        {
            ...
            for (int i = 0; i < values.Length - 1; i++)
            {
                DrawLine(
                    source,
                    (values.Length - 1) - i,
                    (int)(Mathf.InverseLerp(editMin, editMax, values[Mod(currentIndex - i, values.Length)]) * graphHeight),
                    (values.Length - 1) - i - 1,
                    (int)(Mathf.InverseLerp(editMin, editMax, values[Mod(currentIndex - i - 1, values.Length)]) * graphHeight),
                    color
                );
            }
        }
